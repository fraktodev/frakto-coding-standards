...
import { removeSelfClosingSlash } from './utils.js';
...
(async () => {
	...
	if (['format', 'both'].includes(request.mode)) {
		...
		let formatted = await prettier.format(request.content, prettierConfig);
		formatted = removeSelfClosingSlash(formatted);
